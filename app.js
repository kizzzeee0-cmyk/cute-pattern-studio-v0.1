...
function drawBackground(target,w,h){if(state.bg.transparent){target.clearRect(0,0,w,h);return}target.clearRect(0,0,w,h);if(state.bg.mode==='linear'){let a=(state.bg.gradientAngle||135)*Math.PI/180,cx=w/2,cy=h/2,L=Math.abs(w*Math.cos(a))+Math.abs(h*Math.sin(a)),dx=Math.cos(a)*L/2,dy=Math.sin(a)*L/2;let g=target.createLinearGradient(cx-dx,cy-dy,cx+dx,cy+dy);let stops=normalizeGradientStops(state.bg.gradientStops,state.bg.colors);stops.forEach(stop=>g.addColorStop(clamp((stop.pos||0)/100,0,1),stop.color));target.fillStyle=g}else target.fillStyle=state.bg.colors[0];target.fillRect(0,0,w,h);let v=state.bg.vignette||{};if(v.enabled&&v.strength>0){let cx=w/2,cy=h/2;let maxR=Math.sqrt((w/2)*(w/2)+(h/2)*(h/2));let inner=clamp((Number(v.range)||0)/100,0,1);let soft=clamp((Number(v.softness)||0)/100,0,1);let outer=Math.max(inner+0.001,Math.min(1,inner+soft));let g=target.createRadialGradient(cx,cy,maxR*inner,cx,cy,maxR);let alpha=Math.round(255*clamp((Number(v.strength)||0)/100,0,1));let color=normalizeHexLike(v.color||'#6F55C9','#6F55C9');let hex=color.replace('#','');let rr=parseInt(hex.slice(0,2),16),gg=parseInt(hex.slice(2,4),16),bb=parseInt(hex.slice(4,6),16);let rgba=(a)=>`rgba(${rr},${gg},${bb},${a})`;g.addColorStop(Math.min(inner,outer),rgba(0));g.addColorStop(outer,rgba(alpha/255));g.addColorStop(1,rgba(alpha/255));target.save();target.fillStyle=g;target.fillRect(0,0,w,h);target.restore()}}
